...
void buscaProfundidade(Grafo *gr, int ini, int *visitado, int cont, int *caminho){
    int i;
    visitado[ini] = cont; // Informa que o valor ini já foi visitado 
    for(i=0; i<gr->grau[ini]; i++){ // Recebe o grau que cada vertice  contém 
...
void buscaProfundidade_Grafo(Grafo *gr, int ini, int *visitado, int *caminho){
    ...
    buscaProfundidade(gr,ini,visitado,cont, caminho); //Chama a função busca em profundidade
